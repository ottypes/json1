...
transform = type.transform = (oldOp, otherOp, direction) ->
  ...
  drop = (dest, op, other) ->
    ...
    # **** List children
    #
    # TODO: No need for this fanciness if only one of the ops has a .l property.
    if other?.l || op?.l
      opList = op?.l
      otherList = other?.l
...
compose = type.compose = (op1, op2) ->
  checkOp op1
  checkOp op2

  if (debug = type.debug)
    console.log 'op1', JSON.stringify(op1, null, 2)
    console.log 'op2', JSON.stringify(op2, null, 2)

  result = null

  #op1SlotMap = [] # Map from op1's slots -> result's slots.
  #op2SlotMap = [] # Map from op1's slots -> result's slots.

  heldByOp2 = []
  heldByOp2Imm = []

  # 4 parts:
  # - op1.pick gets copied directly
  # - op2.drop gets copied directly
  # - op1.drop gets op2 applied to it
  # - op2.pick gets inverse(op1) applied to it

  grabInImmediate = (c1Imm, c2) ->
    return c1Imm unless c2

    if c2.p != undefined
      heldByOp2Imm[s] = c1Imm if (s = c2.p) != null
      return undefined

    return c1Imm if !c1Imm?

    if c2.o and isObject(c1Imm) then for k, c2Child of c2.o
      result = grabInImmediate c1Imm[k], c2Child
      setO c1Imm, k, result

    if c2.l and Array.isArray(c1Imm) then for k, c2Child of c2.l
      result = grabInImmediate c1Imm[k], c2Child
      setL c1Imm, k, result

    return c1Imm

  grabOp1Drops = (dest, c1, c2) ->
    assert c1? || c2?
    ...
    if hasDrop c1
      ...
      dest ?= {}
      if c1.di != undefined
        dest.di = grabInImmediate clone(c1.di), c2
      else
        dest.d = c1.d

    if c2?.p != undefined
      heldByOp2[s] = dest if (s = c2.p) != null
      return undefined

    # Object children
    if c1?.o
      if c2?.o then for k, c2Child of c2.o
        dest = addOChild dest, k, grabOp1Drops dest, c1.o[k], c2Child

      # And anything that is just in c1.o.
      for k, c1Child of c1.o when !c2?.o?[k]
        dest = addOChild dest, k, grabOp1Drops dest, c1Child, null
  ...
  result = grabOp1Drops result, op1, no, op2 if op1?
