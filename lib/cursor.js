    ...
    read() {
      return {
        ...
        getPath() {
          ...
          // while (i >= 0 || p >= 0) {
          while (i >= 0) {
            // log('unshift', 'p', p, 'i', i, 'c', c, 'c[i]', c[i])
            path.unshift(c[i])
            if (i === 0) {
              i = indexes[p*2]
              c = parents[p--]
        ...
        mergeTree(data, mergeFn = copyAll) {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            if (typeof c === 'string' || typeof c === 'number') {
              ...
              /*
              if (mergePick) {
                if (c.r != undefined) this.write('r', c.r)
                if (c.p != null) this.write('p', c.p)
              }

              if (mergeDrop) {
                // 'i', 'd' and all the edit keys.
                for (let k in c) if (k !== 'r' && k !== 'p') this.write(k, c[k])
              }*/
            }
          }
...
exports.advancer = function(r, listMap, listAdvance) {
  ...
  function adv(k) {
    ...
    while (valid) {
      ...
      //log('advancer', adv._name, k, k2, skip)
      if ((k === k2 && !skip) || isGreaterKey(k2, k)) break
      if (listAdvance && typeof k2 === 'number') listAdvance(k2, r.getComponent())

      valid = r.nextSibling()
    }

    return (valid && k === k2) ? r : null
  }

  adv.end = () => { if (didDescend) r.ascend() }
  return adv
}
