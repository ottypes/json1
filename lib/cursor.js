...
function makeCursor(op = null) {
  ...
  const init = (_container, _idx, _parents, _indexes, _pd) => {
    ...
    op = parents.length ? parents[0] : _container
    if (_pd) pendingDescent = _pd.slice()
  }
  ...
  return {
    ...
    write() {
      ...
      // This is a bit of a dodgy hack. For write cursors to be cloneable, we
      // need to share the operation context into which we're writing.
      // 
      // get() discards this if the cursor is still empty at the end.
      if (op == null) container = op = []
      ...
      return {
        ...
        _init: init, // Internal.
        clone() {
          const c = makeCursor(null).write()
          c._init(container, idx, parents, indexes, pendingDescent)
          return c
        },
        ...
        get() { return op.length === 0 ? null : op },
        ...
        descendPath(path) { pendingDescent.push(...path) },
  ...
  function adv(ktarget) {
    ...
    while (valid) {
      ...
      if (ktarget != null) {
        ...
        if (isGreaterKey(k2, ktarget)) return
        else if (k2 === ktarget && !skip) {
          visited = true
          return r
        }
      }

      // Skip past this item and continue.
      if (onSkip && !visited) onSkip(k2, r)
      visited = false
      if (listAdv && typeof k2 === 'number') listAdv(k2, r.getComponent())
      valid = r.nextSibling()
    }
  }
...
exports.eachChildOf = function(c1, c2, adv1, adv2, fn) {
  ...
  while (hasChild1 || hasChild2) {
    ...
    if (k1 != null && hasChild1) {
      adv1(k1, c1.getComponent())
      hasChild1 = c1.nextSibling()
    }
    if (k2 != null && hasChild2) {
      adv2(k2, c2.getComponent())
      hasChild2 = c2.nextSibling()
    }
  }
