...
const {readCursor, writeCursor, advancer, eachChildOf} = require('./cursor')
...
function traverse(r, w, fn) {
  const c = r.getComponent()
  if (c) fn(c, r, w)

  for (const key of r.children()) {
    if (w) w.descend(key)
    traverse(r, w, fn)
    if (w) w.ascend()
  }
...
const compose = type.compose = function(op1, op2) {
  ...
  traverse(r1, null, (c, p1Pick) => {
    if (c.p != null) held1Pick[c.p] = p1Pick.clone()
  })

  traverse(r2, null, (c, p2Drop) => {
    // Clone the read cursor
    if (c.d != null) held2Drop[c.d] = p2Drop.clone()
  })
  ...
  if (!RELEASE_MODE) checkOp(result)
  return result
}
...
const RM_UNEXPECTED_CONTENT = type.RM_UNEXPECTED_CONTENT = 1
const DROP_COLLISION = type.DROP_COLLISION = 2
const BLACKHOLE = type.BLACKHOLE = 3
...
const tryTransform = type.tryTransform = function(op1, op2, direction) {
  ...
  const op1PickDestinationOfOp2Move = [] // For blackhole detection
  ...
  function scanOp2Drop(r1Pick, r2Pick, r2Drop, pickSlot1, removed1) { // r1Pick, r2Pick could be null.
    ...
    if (c2d) {
      if (((slot2 = c2d.d) != null)) {
        ...
        op1PickDestinationOfOp2Move[slot2] = pickSlot1
        ...
        if (cancelledOp2[slot2]) {
          ...
          if (conflict == null) conflict = {
            ...
            c1: pickConflict(removed1),
            c2: dropConflict(r2Pick, r2Drop)
          }
        }
  ...
  log('op1PickDestinationOfOp2Move', op1PickDestinationOfOp2Move)
  ...
  function writeOp1Pick(r1Pick, r2Pick, r2Drop, w, removed2) {
    ...
    const ap2Pick = advancer(r2Pick, null, (k, c) => {
      if (hasPick(c)) {
        p2PickOff++
  ...
  function writeOp1Drop(p1Pick, p1Drop, p2Pick, p2Drop, w, removed2, insidePick2) {
      ...
      if (c1d.i !== undefined || ((slot1 != null) && pc)) {
        ...
        if (c2d && (c2d.i !== undefined || ((slot2 = c2d.d) != null && !cancelledOp2[c2d.d]))) {
          ...
          if (!identical && (slot2 == null || side === RIGHT || op1PickAtOp2Pick[slot2] == null)) {
            ...
            if (conflict == null) conflict = {
              type: DROP_COLLISION,
              c1: dropConflict(slot1 != null ? heldOp1PickByOp1[slot1] : null, p1Drop),
              c2: dropConflict(slot2 != null ? heldOp2PickByOp2[slot2] : null, p2Drop),
            }
          }
        }
        ...
        if (!identical) {
          if (removed2) {
            ...
            if (conflict == null) conflict = {
              type: RM_UNEXPECTED_CONTENT,
              c1: dropConflict(slot1 != null ? heldOp1PickByOp1[slot1] : null, p1Drop),
              c2: pickConflict(removed2),
            }
          } else {
            // Copy the drops.
            if (slot1 != null) {

              const selfSlot1Parent = insidePick2 != null && op1PickDestinationOfOp2Move[insidePick2]
              if (selfSlot1Parent === slot1) {
                // Blackhole!
                if (conflict == null) conflict = {
                  type: BLACKHOLE,
                  c1: moveConflict(heldOp1PickByOp1[slot1], p1Drop),
                  c2: moveConflict(heldOp2PickByOp2[insidePick2], heldOp2DropByOp2[insidePick2]),
                }
              } else {
    ...
    if (hasPick(c2p)) {
      ...
      if ((c2p.r !== undefined && (!c1p || c1p.r === undefined)) || cancelledOp2[slot2]) {
        ...
        insidePick2 = slot2
        if (op1PickDestinationOfOp2Move[slot2] != null) log('inside pick 1', op1PickDestinationOfOp2Move[slot2])
    ...
    const ap2p = advancer(p2Pick, null, (k, c) => {
    ...
    for (const key of p1Drop.children()) {
      ...
      if (typeof key === 'number') {
        ...
        if (writeOp1Drop(_p1Pick, p1Drop, _p2Pick, _p2Drop, w, removed2, insidePick2)) outDropOff++
        w.ascend()
        ...
        // And recurse.
        w.descend(key)
        writeOp1Drop(_p1Pick, p1Drop, _p2Pick, _p2Drop, w, removed2, insidePick2)
        w.ascend(key)
      }
    }
  ...
  log('----- drop -----')
  writeOp1Drop(r1, r1.clone(), r2, r2.clone(), w, null, null)
  if (conflict) {
    log('returning conflict', conflict)
    return {ok:false, conflict}
  }
  ...
  const eachDrop = (r, w, fn) => traverse(r, w, (c, r, w) => {
  ...
  function writeHeldOp2Drop(p2Drop, outPick, outDrop, w, parentC, removedOut) {
    ...
    if (hasPick(coutp)) {
      if (coutp.p != null) {
        ...
        const slot = coutp.p
        log('teleporting writes to output slot', slot, heldOutDrop[slot].getPath())
    ...
    const oPickAdv = advancer(outPick, null, (k, c) => {
      if (hasPick(c)) {
        outPickOff--
        log('outPickOff++')
      }
  ...
  if ((heldDropWrites.length || cancelledOp2.length) && !conflict) {
    ...
    if (!RELEASE_MODE) {
      ...
      log('heldOutDrop', heldOutDrop.map(r => r && r.getPath()))
    }
    ...
    if (!RELEASE_MODE) log('held output writes', heldWritesOut.map(h => h.get()))
    if (heldWritesOut.length) {
      const rOut2 = readCursor(shallowCloneOp(w.get()))
      eachDrop(rOut2, w, (slotOut, r, w) => {
        const _w = heldWritesOut[slotOut]
        if (_w) w.mergeTree(_w.get())
      })
    }
  }
...
//
// TODO(cleanup): Move these out.
const removeOp = type.removeOp = (path, value = true) => (
  writeCursor()
  .writeAtPath(path, 'r', value)
  .get()
)

const mvOp = type.mvOp = (from, to) => (
  writeCursor()
  .writeAtPath(from, 'p', 0)
  .writeAtPath(to, 'd', 0)
  .get()
)

const insertOp = type.insertOp = (path, value) => (
  writeCursor()
  .writeAtPath(path, 'i', value)
  .get()
)

const replaceOp = type.replaceOp = (path, oldVal, newVal) => (
  writeCursor().at(path, () => {
    w.write('r', oldVal)
    w.write('i', newVal)
  }).get()
...
const editOp = type.editOp = (path, type, subOp) => (
  writeCursor()
  .at(path, () => writeEdit(w, type, subOp))
  .get()
)
...
const resolveConflict = (conflict, side) => {
  ...
  switch (type) {
    case DROP_COLLISION:
      ...
      return side === 'left'
        ? [null, removeOp(c2.drop)]
        : [removeOp(c1.drop), null]

    case RM_UNEXPECTED_CONTENT:
      return c1.type === 'remove' ? [null, removeOp(c2.drop)] : [removeOp(c1.drop), null]
    ...
    case BLACKHOLE:
      ...
      return [removeOp(c1.drop), removeOp(c2.drop)]
