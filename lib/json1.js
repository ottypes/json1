...
const getEditType = (c) => (
  (c == null) ? null
    ...
    : c.ena ? subtypes.number
    : null
)

const getEdit = c => c.es || c.ena || c.e
  ...
  function xfBoundary(r1Pick, r1Drop, r2Pick, r2Drop, litIn, rmParent, wd, wp) {
    ...
    eachChildOf(r1Drop, r2Pick, (k, c) => {
      ...
      if (typeof inKey === 'number') {
        ...
        litKey = inKey + p2PickOff + p1DropOff
      ...
      log('->', 'p1pick', p1PickKey, 'inkey', inKey, 'p2drop', p2DropKey)
...
const tryTransform = type.tryTransform = function(op1, op2, direction) {
  ...
  function writeHeldOp2Drop(p2Drop, outPick, outDrop, w, parentC, removedOut) {
    ...
    if (hasPick(coutp)) {
      if (coutp.p != null) {
        ...
        const slot = coutp.p
        log('teleporting writes to output slot', slot, heldOutDrop[slot])
    ...
    if (c2) {
      ...
      if ((slot2 = c2.d) != null) {
        ...
        if (_w) {
          ...
          if (!RELEASE_MODE) log('checking for blackholed', _w.get(), parentC && parentC.p)
          let slot1
          if (parentC && (slot1 = getSelfParentSlot(readCursor(_w.get()))) != null) {
            ...
            if (conflict == null) conflict = {
              ...
              c1: moveConflict(heldOp1PickByOp1[slot1], heldOp1DropByOp1[slot1]),
  ...
  if ((heldDropWrites.length || cancelledOp2.length) && !conflict) {
    ...
    if (!RELEASE_MODE) log('merging writes into', rOut.get())
