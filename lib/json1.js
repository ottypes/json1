...
type.compose = function(op1, op2) {
  ...
  // This is called as a helper for xfBoundary, so if op2 pick descends into
  //function xfOp1Literal(
  ...
  function xfBoundary(r1Pick, r1Drop, r2Pick, r2Drop, litIn, rmParent, wd, wp) {
    ...
    const dropSlot = c1d ? c1d.d : null
    const pickSlot = c2p ? c2p.p : null

    log('pickSlot', pickSlot, 'dropSlot', dropSlot)

    // rmParent arg for when we recurse.
    const rmChildren = (rmParent || rmHere) && (pickSlot == null)

    if (pickSlot != null) {
      log('teleporting via c2 pickSlot', pickSlot)
      r2Drop = held2Drop[pickSlot]

      // Writes here need to be transformed to op2's drop location.
      wd = heldDropWrites[pickSlot] = writeCursor()
      //literal = undefined
    }

    const c2d = getComponent(r2Drop)

    if (dropSlot != null) { // Drop here.
      log('teleporting via c1 dropSlot', dropSlot)
      r1Pick = held1Pick[dropSlot]

      // Writes here need to be transformed to op2's drop location.
      wp = heldPickWrites[dropSlot] = writeCursor()

      if (!rmParent && !rmHere) {
        const slot = p1SlotMap[dropSlot] = nextSlot++
        log('assigning slot', slot, 'to op1 slot', dropSlot)
        wd.write('d', slot)
      } else {
        log('Cancelling op1 move', dropSlot)
    ...
    if (pickSlot != null) {
      ...
      if (litIn !== undefined || insHere) {
        ...
        const slot = dropSlot != null ? p1SlotMap[dropSlot] : nextSlot++

        p2SlotMap[pickSlot] = slot
        log('assigning slot', slot, 'to op2 slot', dropSlot)
        wp.write('p', slot)
      }
    } else if (rmHere) {
    ...
    const type1 = (!rmParent && !rmHere) ? getType(c1d) : null
    const type2 = getType(c2d)
    ...
    const insComponent =
      (insHere && !rmParent && !rmHere && pickSlot == null)
        || (pickSlot != null && litOut !== undefined) ? wd.getComponent() : null
    ...
    const hasContainerLiteral = typeof litOut === 'object'
    ...
    if (insComponent != null) {
      ...
      litOut = litIn
      //log('a', litIn)
    } else if (!rmHere && !rmParent && pickSlot == null) {
      //log('b', litOut)
      return litOut
    } else {
      //log('c', undefined)
...
const transform = type.transform = function(op1, op2, direction, opts) {
  ...
  const heldOp1Pick = [] // Cloned *read* cursors for teleporting, indexed by p1.
  const heldOp2Drop = []
  ...
  function writeOp1Drop(p1Pick, p1Drop, p2Pick, p2Drop, w, removed2) {
    ...
    if (c1d) {
      const slot1 = c1d.d
      if (slot1 != null) {
        log('teleporting p1Pick via op1 slot', slot1)
    ...
    if (c2p) {
      ...
      if (slot != null) {
        // Teleport.
        log('teleporting p2Drop via c2 slot', slot)
    ...
    // needs to be transformed by op2's drop (if any), but we're not currently
    // tracking the op2 drop location in enough detail to accurately spot it.
    //
    // I'll write the edit out directly in the output here and do the transform
    // in writeOp2Drop below.
  ...
  // as output picks.
  function writeOp2Pick(p2Drop, outPick, w, removedOut) {
    ...
    p2Drop.eachChild(key => {
      w.descend(key)
      writeOp2Pick(p2Drop, oPickAdv(key), w, removedOut)
      w.ascend()
    })
    oPickAdv.end()
  }

  const transformEditInto = (c1, c2, w) => {
    const t1 = getType(c1)
    if (t1) {
      const e1 = getEdit(c1)
      const t2 = getType(c2)
  ...
  function writeOp2Drop(p2Drop, outPick, outDrop, w, parentC) {
    log('writeOp2Drop', parentC)
    ...
    if (c2) {
      ...
      //const t2 = getType(c)
      //if (t2) {
        // Op2 also has an edit, and we need to check if there's an edit
        // sitting in the writer that we need to transform out.
    ...
    p2Drop.eachChild(o2dk => {
      if (typeof o2dk === 'number') {
        ...
        writeOp2Drop(p2Drop, _outPick, _outDrop, w, parentC)
        w.ascend()

      } else {
        w.descend(o2dk)
        writeOp2Drop(p2Drop, oPickAdv(o2dk), oDropAdv(o2dk), w, parentC)

        // TODO: This is super wrong, but it doesn't fail any of my tests.
        //writeOp2Drop(p2Drop, outPick, outDrop, w, parentC)
        w.ascend()
      }
    })
  ...
  if (heldWrites.length || cancelledOp2.length || overwrittenDrops) {
    ...
    writeOp2Pick(r2, rOut2, w, false)
    w.reset()
    log('-- after writeOp2Pick', w.get())

    const rOut = readCursor(shallowCloneOp(w.get()))
    writeOp2Drop(r2, rOut, rOut.clone(), w, null)
  }
...
if (require.main === module) {
  ...
  // transform([ [ 'eyes', { p: 0 } ], [ 'wabe', { d: 0, es: [ 2, 'awhile Beware ' ] } ] ],
  //   [ 'eyes', { es: [ 1, { d: 1 }, 'as ' ] } ], 'left')

  debugger
  type.compose([ [ 0, { p: 0, d: 1 } ], [ 1, { p: 1 } ], [ 2, { d: 0 } ] ], [ [ 0, { p: 0 } ], [ 1, { d: 0 } ] ])
  // type.compose([ [ 0, { p: 0 } ], [ 2, { d: 0 } ] ], [ [ 0, { p: 0 } ], [ 1, { d: 0 } ] ])
