...
const transform = type.transform = function(oldOp, otherOp, direction) {
  ...
  const r1 = readCursor(oldOp)
  const r2 = readCursor(otherOp)
  ...
  function scanOp2DropAndCancel(r2, r1, w, removed) { // r1 might be null.
    ...
    if (c2 && ((slot = c2.d) != null)) {
      ...
      if (!removed && cancelledOp2[slot]) {
        ...
        w.write('r', true) // A move into a deleted subtree. -> Remove.
        removed = true
      }
    }
  ...
  function opPick(r1Pick, r2Pick, r2Drop, w, removed) {
    ...
    if (c1) {
      ...
      } else if (c1.r !== undefined && !removed) {
  ...
  log('hp', heldOp1Pick.map(x => x.getComponent()))
  ...
  function opDrop(p1Pick, p1Drop, p2Pick, p2Drop, w, removed) {
    ...
    p1Drop.eachChild(key => {
      ...
      if (typeof key === 'string') {
        ...
        const k1Mid = key - p1DropOff;

        { // Calculate _p1Pick.
          let p1k
          while (p1pValid && typeof (p1k = p1Pick.getKey()) === 'number') {
            p1k -= p1PickOff;
            const hp = hasPick(p1Pick.getComponent());
        ...
        { // Calculate _p2Drop.
          while (p2dValid && typeof (p2dk = p2Drop.getKey()) === 'number') {
            ...
            if (hd) {
              ...
              const slot = c.d
              if ((c.d != null)
                  && (cancelledOp2[slot] || ((op1PickAtOp2Pick[slot] != null) && side === LEFT))) {
                outPickOff++
              }
