...
const assert = !RELEASE_MODE ? require('assert') : (pred, msg) => {if (!pred) throw new Error(msg)}
...
const removeOp = exports.removeOp = (path, value = true) => (
  writeCursor()
  .writeAtPath(path, 'r', value)
  .get()
)

const mvOp = exports.mvOp = (from, to) => (
  writeCursor()
  .writeMove(from, to)
  .get()
)

const insertOp = exports.insertOp = (path, value) => (
  writeCursor()
  .writeAtPath(path, 'i', value)
  .get()
)

const replaceOp = exports.replaceOp = (path, oldVal, newVal) => (
  writeCursor().at(path, () => {
    w.write('r', oldVal)
    w.write('i', newVal)
  }).get()
...
const editOp = exports.editOp = (path, type, subOp) => (
  writeCursor()
  .at(path, w => writeEdit(w, type, subOp))
  .get()
)

// Helper function which trims anything underneath a remove
// type.trim = (op) => {
//   const w = writeCursor()
...
const normalize = type.normalize = function(op) {
  let nextSlot = 0
  let slotMap = []
...
const incPrefix = () => { if (!RELEASE_MODE) log.prefix++ }
const decPrefix = () => { if (!RELEASE_MODE) log.prefix-- }

type.applyPath = (path, op) => {
  log('applyPath', path, op)
  incPrefix()
  path = path.slice()
  checkOp(op)
  ...
  for (let i = 0;; i++) {
    ...
    let pickOffset = 0
    const pickAdv = advancer(r, null, (k, c) => {
      if (hasPick(c)) {
        pickOffset++
    ...
    if (typeof k === 'number') path[i] -= pickOffset
    if (!hasNext) break
  }
  ...
  const handleDrop = () => {
    ...
    // way based on earlier list drops.
    for (; i < path.length; i++) {
      const k = path[i]
      ...
      if (typeof k === 'number') path[i] += dropOffset
...
const tryTransform = type.tryTransform = function(op1, op2, direction) {
      ...
      if (((slot2 = c2d.d) != null)) {
        ...
        if (cancelledOp2[slot2]) {
          ...
          if (conflict == null) conflict = {
            ...
            op2: mvOp(r2Pick.getPath(), r2Drop.getPath())
          }
        }
  ...
  function writeOp1Drop(p1Pick, p1Drop, p2Pick, p2Drop, w, removed2 /*, insidePick2*/) {
    ...
    const insOrMv = (r1, r2, c) => r1 ? mvOp(r1.getPath(), r2.getPath()) : insertOp(r2.getPath(), c.i)
      ...
      if (c1d.i !== undefined || ((slot1 != null) && pc)) {
        ...
        if (c2d && (c2d.i !== undefined || ((slot2 = c2d.d) != null && !cancelledOp2[c2d.d]))) {
          ...
          if (!identical && (slot2 == null || side === RIGHT || op1PickAtOp2Pick[slot2] == null)) {
            ...
            if (conflict == null) conflict = {
              type: DROP_COLLISION,
              op1: insOrMv(slot1 != null && heldOp1PickByOp1[slot1], p1Drop, c1d),
              op2: insOrMv(slot2 != null && heldOp2PickByOp2[slot2], p2Drop, c2d),
            }
          }
        }
  ...
  while (true) {
    ...
    if (ok) return compose(r2Aggregate, result)
      ...
      if (!RELEASE_MODE && conflict.type === BLACKHOLE) {
        const {ok: ok2, result: result2, conflict: conflict2} = tryTransform(op2, op1, side === 'left' ? 'right' : 'left')
        assert(!ok2)
        try {
          assert.deepStrictEqual(normalizeConflict(conflict2), normalizeConflict(invConflict(conflict)))
        } catch (e) {
          log.debug = true
          log('Inverted transform conflict', op1, op2, conflict, conflict2)
          throw e
        }
      }
