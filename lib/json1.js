...
const transform = type.transform = function(op1, op2, direction, opts) {
  ...
  //
  // removed tracks op1's removals.
  function scanOp2DropAndCancel(r2Drop, r1Pick, w, removed) {
    // r1Pick might be null.
    ...
    r2Drop.eachChild(key => {
      w.descend(key)
      scanOp2DropAndCancel(r2Drop, ap1(key), w, removed)
      w.ascend(key)
    })
    ap1.end()
  }
  ...
  // to stage any operations that have been moved by op2.
  const heldWrites = []
      ...
      if (typeof key === 'number') {
        ...
        { // Calculate _p2Drop.
          ...
          while (p2dValid && typeof (p2dk = p2Drop.getKey()) === 'number') {
            ...
            const hd = hasDrop(c)
            log('p2d scan', op2Mid, p2DropOff, k2Mid, hd, iHaveDrop)

            if (op2Mid > k2Mid) break
            // This works, but is harder to read:
            // break if op2Mid == k2Mid and (!hd or (side is LEFT and iHaveDrop))
            if (op2Mid === k2Mid) {
  ...
  function writeHeldPicks(p2Drop, w) {
    ...
    if (c2 && (c2.d != null)) {
      ...
      if (_w) w.mergeTree(_w.get(), true, false)
    }

    p2Drop.eachChild(key => {
      w.descend(key)
      writeHeldPicks(p2Drop, w)
      w.ascend()
    })
  }
  writeHeldPicks(r2, w)
  ...
  function writeHeldDrops(p2Drop, outPick, outDrop, w) {
    ...
    if (c2 && (c2.d != null)) {
      const _w = heldWrites[c2.d]
      if (_w) w.mergeTree(_w.get(), false, true)
    }
  ...
  // This is pretty nasty. The problem is that we need a read cursor over the
  // output so far, but the output is still mutable (we're going to mutate it).
