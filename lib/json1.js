...
const tryTransform = type.tryTransform = function(op1, op2, direction) {
  ...
  const uniqDesc = (key, reader) => {
    ...
    const path = reader.getPath().map((k,i) => `${k}:i`).join('.')
  ...
  function writeOp1Drop(p1Pick, p1Drop, p2Pick, p2Drop, w, removed2) {
    ...
    // uniqDesc('op1drop r2Pick', p2Pick)
    // uniqDesc('op1drop r2Drop', p2Drop)
    // uniqDesc('op1drop r1Pick', p1Pick)
    uniqDesc('op1drop r1Drop', p1Drop)
