...
const apply = type.apply = function(snapshot, op) {
...
const transform = type.transform = function(op1, op2, direction, opts) {
  ...
  // Map from op2's slots -> true if the moved item's container has been
  // removed by op1.
  const cancelledOp2 = []
  ...
  function scanOp2Pick(r2Pick, r1Pick, removed1) { // r1Pick might be null.
    log('scanOp2Pick', r1Pick && 'r1Pick', 'removed1:', removed1)
    log.prefix++
  ...
  function scanOp2Drop(r1Pick, r2Pick, r2Drop, removed1) { // r1Pick, r2Pick could be null.
    log('scanOp2Drop', r1Pick && 'r1Pick', r2Pick && 'r2Pick', 'removed:', removed1)
    log.prefix++
    ...
    if (c2d) {
      if (((slot = c2d.d) != null)) {
        ...
        // Teleport r2Pick
        // r2Pick = heldOp2Pick[slot] ? heldOp2Pick[slot].clone() : null
        log('tele r2Pick to op2 slot', slot, !!cancelledOp2[slot])
        r1Pick = heldOp1PickByOp2[slot]
        r2Pick = heldOp2PickByOp2[slot]
        if (cancelledOp2[slot]) removed1 = true

        // I'm reusing the cancelled op2 slots here. Not sure if this is correct.
        if (removed1) {
          log('cancelling op2 move because drop destination removed', slot)
          cancelledOp2[slot] = true
        }
    ...
    const c1p = getComponent(r1Pick)
    if (c1p) {
      if (c1p.r !== undefined) removed1 = true
      else if (c1p.p != null) removed1 = false
    }
  ...
  scanOp2Drop(r1, r2, r2.clone(), false)
  ...
  function writeOp1Drop(p1Pick, p1Drop, p2Pick, p2Drop, w, removed2) {
    ...
    log('drop', !!p1Pick, !!p1Drop, !!p2Pick, !!p2Drop, 'c:', c1d, 'r:', removed2)
    log.prefix++
      ...
      if (c1d.i !== undefined || ((slot1 != null) && pc)) {
        ...
        if (!identical) {
          if (!removed2 && write) {
            ...
            // We hit here if removed2 is true or write is false. This logic seems wrong??
        ...
        { // Calculate _p1Pick and outPickOff
          ...
          while (p1pValid && typeof (p1k = p1Pick.getKey()) === 'number') {
            ...
            if (hp) {
              ...
              log(c.d != null, getComponent(heldWrites[c.d]), hasPick(getComponent(heldWrites[c.d])))
  ...
  // This function iterates through the p2 picks
  function writeHeldOp2Pick(p2Drop, outPick, w, removedOut) {
    ...
    if (c2) {
      ...
      if (slot != null) {
        ...
        if (_w) w.mergeTree(_w.get(), (c, w) => {
          ...
          if (c.r !== undefined) w.write('r', true) // TODO: c.r
    ...
    const c3 = getComponent(outPick)
    if (c3 && c3.r !== undefined) {
      removedOut = true
    } // If c3 contains a pick, it doesn't matter.
  ...
  function writeHeldOp2Drop(p2Drop, outPick, outDrop, w, parentC, removedOut) {
    log('writeHeldOp2Drop', !!outPick, !!outDrop, 'pc', parentC, 'rm out', removedOut)
    const cout = getComponent(outPick)
    if (cout) {
      if (cout.p != null) {
        parentC = cout
        ...
        if (_w) {
          ...
          const checkSelfParent = (r) => {
            ...
            for (const k of r.children()) {
              if (checkSelfParent(r)) return true
            }
            return false
          }
          log('checking for blackholed', _w.get(), parentC && parentC.p)
          if (parentC && checkSelfParent(readCursor(_w.get()))) {
            log('Cancelling blackholed', parentC.p)
            delete parentC.p
            parentC.r = true
          } else {
            // If removedOut is set and the merge tree contains any inserts or
            // drops, we've lost track of a remove along the way. Drops should
            // have been cancelled before this point.
            w.mergeTree(_w.get(), (c, w) => {
              // merge drops and edits
              if (removedOut && (hasDrop(c) || getEditType(c))) throw Error('Cant merge into removed area')
    ...
    for (const o2dk of p2Drop.children()) {
      if (typeof o2dk === 'number') {
        ...
        log('->', o2dk, 'outPickOff', outPickOff, 'dropOff', outDropOff, 'final', rmid + outDropOff)
  ...
  // Am I going to need a pick variant of this too?
  function writeHeldOutputDrop(outDrop, w) {
