...
const tryTransform = type.tryTransform = function(op1, op2, direction) {
  ...
  // 
  // Either r1Pick or r2Drop will always be set, but it could be either one.
  function writeOp1Pick(r1Pick, r2Pick, r2Drop, w, w2, removed2) {
    log('writeOp1Pick', 'r1Pick', r1Pick && r1Pick.getPath(), 'r2Pick', r2Pick && r2Pick.getPath(), 
    ...
    if (c2p) {
      ...
      if (slot != null) {
        ...
        if (!(w = heldPickWrites[slot])) w = heldPickWrites[slot] = writeCursor()
        iAmMoved = true
        removed2 = null
    ...
    if (c2d) {
      const slot2 = c2d.d
      if (slot2 != null) {
        log('holding write pick dest for slot2', slot2)
        writePickDest[slot2] = w2.clone()
        // if (cancelledOp2[slot2]) w2.write('r', true)
        if (cancelledOp2[slot2] && !discardedOp2Drop[slot2]) w2.write('r', true)
      }
    }
    ...
    const ap2Drop = cursor.advancer(r2Drop, (k, c) => (
      ...
      log('traversing into r2Drop to find my write bunny', k2, p2Drop_.getPath(), p2DropOff)
      w.descend(k2)
      w2.descend(k2) // ??? TODO: k2? key? Adjust?? Fix me.
      writeOp1Pick(null, null, p2Drop_, w, w2, null)
      w2.ascend()
      w.ascend()
    })
    ap2Drop._name = '2drop'
    ...
    if (r1Pick) for (const key of r1Pick.children()) {
      if (typeof key === 'string') {
        ...
        w.descend(key)
        w2.descend(key)
        writeOp1Pick(r1Pick, p2Pick_, p2Drop_, w, w2, removed2)
        w2.ascend()
        w.ascend()
      } else {
        const p2Pick_ = ap2Pick(key)
  ...
  log('---- pick ----')
  const w2 = writeCursor()
  writeOp1Pick(r1, r2, r2.clone(), w, w2, null)
  w.reset()
  ...
  function writeHeldOp2Drop(p2Drop, outPick, outDrop, w, parentC, removedOut) {
    ...
    for (const o2dk of p2Drop.children()) {
      if (typeof o2dk === 'number') {
        ...
        log('->', o2dk, 'outPickOff', outPickOff, '-> rmid', rmid, 'dropOff', outDropOff, 'final', rmid + outDropOff)
  ...
  if ((heldDropWrites.length || cancelledOp2.length) && !conflict) {
    ...
    log('held writes', heldDropWrites.map(h => h.get()))
